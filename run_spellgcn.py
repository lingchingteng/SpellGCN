...
flags.DEFINE_string("graph_dir", None, "The dir for graph data")
...
flags.DEFINE_integer(
    "max_nodes", 4755, "Maximum number of adjacent nodes. "
    "Must match data generation.")

flags.DEFINE_integer("max_relations", 2, "Maximum number relations.")
flags.DEFINE_integer(
    "next_sent_type", 3,
...
flags.DEFINE_bool("use_horovod", False,
                  "Whether to use Horovod for multi-gpu training.")
...
class LossHook(tf.train.SessionRunHook):
    def __init__(self, intervel):
        ...
        self.step = 0
        self.fetches = tf.train.SessionRunArgs(fetches=[
            "global_step:0", "total_loss:0", "masked_lm_loss:0",
            "learning_rate:0"
        ])
    ...
    def after_run(self, run_context, run_values):
        if self.step % self.intervel == 0:
            global_step, total_loss, masked_lm_loss, learning_rate = run_values.results
            tf.logging.info(
                'global_step=%d\ttotal_loss=%2.6f\tmasked_lm_loss=%2.6f\tlearning_rate=%.6e'
...
def gcnLayer(gcn_in,
    ...
    for layer in range(num_layers):
        ...
        with tf.name_scope("%s-%d" % (name, layer)):
            # with tf.variable_scope("Loop-name-%s-layer-%d" %
            #                        (name, layer)) as scope:
            #     w_loop = tf.get_variable("w_loop",
            #                              initializer=tf.eye(in_dim),
            #                              trainable=False)
            #     inp_loop = tf.tensordot(gcn_in, w_loop, axes=[1, 0])
            #     if dropout != 1.0:
            #         inp_loop = tf.nn.dropout(inp_loop, keep_probs=dropout)
            #     loop_act = inp_loop
            act_sum = out[-1]
            ...
            for lbl in range(FLAGS.max_relations):
                with tf.variable_scope("label-%d-name-%s_layer-%d" %
                                       (lbl, name, layer),
                                       reuse=tf.AUTO_REUSE) as _:
                    w_in = tf.get_variable(
                        'w_in',
                        initializer=tf.eye(gcn_dim),
                        trainable=True,
                    )
                    ...
                    if w_gating:
                        w_gin = tf.get_variable(
                            'w_gin', [gcn_dim, 1],
                            initializer=tf.contrib.layers.xavier_initializer())
                        b_gin = tf.get_variable(
                            'b_gin', [1], initializer=tf.zeros_initializer())

                with tf.name_scope('arcs-%d_name-%s_layer-%d' %
                                   (lbl, name, layer)):
                    inp_in = tf.matmul(gcn_in, w_in) + b_in  # [n, d]
                    if adj_mat is not None:
                        adj_matrix = adj_mat[lbl]
                        in_t = tf.matmul(tf.cast(adj_matrix, tf.float32),
                                         inp_in)  # [n, d]
                    else:
                        in_t = inp_in

                    if dropout != 1.0:
                        in_t = tf.nn.dropout(in_t, keep_prob=dropout)
                    if w_gating and adj_mat is not None:
                        inp_gin = tf.sigmoid(
                            tf.matmul(gcn_in, w_gin) +
                            tf.to_float(b_gin[None, :]))  # [n, 1]
                        in_act = tf.matmul(tf.cast(adj_matrix, tf.float32),
                                           inp_gin * inp_in)  # [n, d]
                    else:
                        in_act = in_t
                if lbl == 0:
                    ...
                    multi_graph_embs = tf.concat(
                        [multi_graph_embs, in_act[None, :, :]],
                        axis=0)  # [MAX_REL, n, d]

            with tf.variable_scope("relation_prototype_scope",
                                   reuse=tf.AUTO_REUSE):
                w0 = tf.get_variable(
                    "relation_prototype",
                    shape=[in_dim, 1],
                ...
                _act_sum = tf.reduce_sum(graph_weights * multi_graph_embs,
                                         axis=0)
...
def build_gcn_output(adj_mat, w2n, n2w, embedding_table, bert_config,
                     is_training):
    for lbl in range(FLAGS.max_relations):
        adj = adj_mat[lbl]
    ...
    with tf.variable_scope("gcn"):
        node_embedding = gcnLayer(
            gcn_in=tf.gather(
                embedding_table,
                _n2w),  # n2w'size is FLAGS.nodes, max value is 21127
    ...
    #         gcn_in=model.get_embedding_table(
    #         ),  # n2w'size is FLAGS.nodes, max value is 21127
    #         in_dim=bert_config.hidden_size,
    #         gcn_dim=bert_config.hidden_size,
    #         batch_size=32,
    #         max_nodes=FLAGS.max_nodes,
    ...
    expanded_node_embedding = tf.gather(
        node_embedding, _w2n)  # w2n's is is 21127, max value is FLAGS.nodes
    mask_nodes_ids = tf.to_float(tf.not_equal(_w2n, 0))
    gcn_embedding = mask_nodes_ids[:, None] * expanded_node_embedding + (
        1 - mask_nodes_ids[:, None]) * rest_embedding

    return gcn_embedding


def model_fn_builder(adj_mat, w2n, n2w, bert_config, init_checkpoint,
    ...
    def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
        ...
        gcn_embedding = build_gcn_output(adj_mat, w2n, n2w,
...
class CSCProcessor(DataProcessor):
    ...
    def get_dev_full_examples(self, data_dir):
        ...
        examples = []
        for i, (inp, trth) in enumerate(zip(open(input_file),
                                            open(truth_file))):
            ...
            for j, pos_tok in enumerate(trth_fields[1:]):
                if j % 2 == 0:
                    ...
                    if int(pos) < FLAGS.max_seq_length and int(pos) <= len(
                            masked_lm_tokens):
                        masked_lm_tokens[int(pos) -
                                         1] = pos_tok.strip(",").decode("utf8")
            examples.append(
...
def convert_single_example(ex_index, example, label_list, output_mode,
...
def file_based_convert_examples_to_features(examples, label_list, output_mode,
                                            max_seq_length,
...
def file_based_input_fn_builder(input_file, seq_length,
                                max_predictions_per_seq, is_training,
    ...
    with open("%s/relation_vocab.txt" % (FLAGS.graph_dir)) as f:
        for i, line in enumerate(f):
            if len(line.split(" ")) == 1:
                ...
                rels_vocab.setdefault(line.strip().split(" ")[0],
                                      int(line.strip().split(" ")[1]))
    np_adj_mat = np.zeros(
        [len(set(rels_vocab.values())), FLAGS.max_nodes, FLAGS.max_nodes],
        dtype=np.float)
    with open("%s/spellGraphs.txt" % (FLAGS.graph_dir)) as f:
        for i, line in enumerate(f):
            e1, e2, rel = line.strip().split("|")
            if rel in rels_vocab:
                np_adj_mat[rels_vocab[rel], nodes_vocab[e1],
                           nodes_vocab[e2]] = 1
                np_adj_mat[rels_vocab[rel], nodes_vocab[e2],
                           nodes_vocab[e1]] = 1
...
def main(_):
    ...
    if FLAGS.do_train:
        train_file = os.path.join(
            FLAGS.output_dir,
            "train{0}.tf_record".format(FLAGS.max_seq_length))
        label_list = processor.get_labels()
        tokenizer = tokenization.FullTokenizer(
            vocab_file=FLAGS.vocab_file, do_lower_case=FLAGS.do_lower_case)
        if not os.path.exists(train_file):
            file_based_convert_examples_to_features(
                train_examples, label_list, processor.get_mode(),
                FLAGS.max_seq_length, FLAGS.max_predictions_per_seq, tokenizer,
                train_file)
        ...
        eval_file = os.path.join(
            FLAGS.output_dir, "eval{0}.tf_record".format(FLAGS.max_seq_length))
        label_list = processor.get_labels()
        tokenizer = tokenization.FullTokenizer(
            vocab_file=FLAGS.vocab_file, do_lower_case=FLAGS.do_lower_case)
        ...
        test_file = os.path.join(
            FLAGS.output_dir, "test{0}.tf_record".format(FLAGS.max_seq_length))
        label_list = processor.get_labels()
        tokenizer = tokenization.FullTokenizer(
            vocab_file=FLAGS.vocab_file, do_lower_case=FLAGS.do_lower_case)
            ...
            with tf.gfile.GFile(input_file, "w") as input_writer:
                ...
                for i, res in enumerate(result):
                    if i % (FLAGS.max_seq_length) == 0:
                        ...
                        log_prob_row = []
                        input_writer.write("%s\t" % (test_examples[int(
                            i / FLAGS.max_seq_length)].guid))
        ...
        correction_file = os.path.join(FLAGS.output_dir,
                                       "correction_results.txt")
        correction_writer = tf.gfile.GFile(correction_file, "w")
        for idx, (pred, actual) in enumerate(
                zip(
                    open(labels_file),
                    open("%s/TestTruthWithError.txt" % FLAGS.data_dir)
                    if FLAGS.with_error else open("%s/TestTruth.txt" %
                                                  FLAGS.data_dir))):
            ...
            correct_actual_zip = zip(detect_actual_tokens,
                                     correct_actual_tokens)
            ...
            correct_TP += len(
                set(correct_pred_tokens)
                & set(zip(detect_actual_tokens, correct_actual_tokens)))
            correct_FP += len(
                set(correct_pred_tokens) -
                set(zip(detect_actual_tokens, correct_actual_tokens)))
            correct_FN += len(
        ...
        if FLAGS.with_error:
            ...
            print(
                "dc_joint_precision=%f, dc_joint_recall=%f, dc_joint_Fscore=%f"
                % (dc_precision, dc_recall, dc_F1))
        ...
        if not FLAGS.with_error:
            ...
            print(
                "detect_sent_precision=%f, detect_sent_recall=%f, detect_Fscore=%f"
                % (detect_sent_precision, detect_sent_recall, detect_sent_F1))
            print(
                "correct_sent_precision=%f, correct_sent_recall=%f, correct_Fscore=%f"
                %
